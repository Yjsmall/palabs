...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                                                                                         \
    ...
    // logic operation
    INSTPAT("0000000 ????? ????? 000 ????? 011 0011", add, R, printf("---------add"); debug_print(src1, src1, imm); R(rd) = src1 + src2;);
    ...
    INSTPAT("0000000 ????? ????? 111 ????? 011 0011", and_log, R, R(rd) = src1 & src2;);
    // mul operation
    INSTPAT("0000001 ????? ????? 000 ????? 011 0011", mul, R, R(rd) = src1 * src2;);
    INSTPAT("0000001 ????? ????? 110 ????? 011 0011", rem, R, debug_print(src1, src2, imm); printf("%d\n", src1 % src2); R(rd) = 1;
            R(rd) = src1 % src2;);
    ...
    INSTPAT("0000000 ????? ????? 001 ????? 0010011", slli, I, int shamt = imm & 0x1f; R(rd) = src1 << shamt;);
    ...
    INSTPAT("0100000 ????? ????? 101 ????? 0010011", srai, I, int shamt = imm & 0x1f; R(rd) = (int32_t)src1 >> shamt;);
    ...
    //                    rs2   rs1            opcode
    // INSTPAT("??????? ????? ????? 000 ????? 110 0011", beq, B, s->dnpc = ((int32_t)src1 ==(int32_t) src2) ? s->dnpc + imm : s->dnpc + 4;);
    // INSTPAT("??????? ????? ????? 001 ????? 110 0011", bne, B, s->dnpc = ((int32_t)src1 !=(int32_t) src2) ? s->dnpc + imm : s->dnpc + 4;);
    // INSTPAT("??????? ????? ????? 100 ????? 110 0011", blt, B, s->dnpc = ((int32_t)src1 < (int32_t) src2) ? s->dnpc + imm : s->dnpc + 4;);
    // INSTPAT("??????? ????? ????? 101 ????? 110 0011", bge, B, s->dnpc = ((int32_t)src1 >=(int32_t) src2) ? s->dnpc + imm : s->dnpc + 4;);
    INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B, if (src1 == src2) s->dnpc = s->pc + imm);
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B, printf("bne\n"); debug_print(src1, src2, imm); if (src1 != src2) s->dnpc = s->pc + imm);
