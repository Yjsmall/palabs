...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                                                                                         \
    ...
    //                    rs2   rs1            opcode
    INSTPAT("??????? ????? ????? 000 ????? 110 0011", beq, B, s->dnpc = (src1 == src2) ? s->dnpc + imm : s->dnpc + 4;);
    INSTPAT("??????? ????? ????? 001 ????? 110 0011", bne, B, s->dnpc = (src1 != src2) ? s->dnpc + imm : s->dnpc + 4;);
    INSTPAT("??????? ????? ????? 101 ????? 110 0011", bge, B, s->dnpc = (src1 >= src2) ? s->dnpc + imm : s->dnpc + 4;);
