...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                                                                                         \
    ...
    INSTPAT("??????? ????? ????? 111 ????? 0010011", addi, I, R(rd) = src1 & imm);
    ...
    INSTPAT("0000000 ????? ????? 110 ????? 011 0011", and, R, R(rd) = src1 | src2;);
    INSTPAT("0000000 ????? ????? 111 ????? 011 0011", and, R, R(rd) = src1 & src2;);

    // TYPE_B: B-type instruction
    //                    rs2   rs1             opcode
    INSTPAT("??????? ????? ????? 000 ????? 11000 11", bne, B, s->dnpc = (src1 == src2) ? s->dnpc + imm : s->pc + 4;);
    INSTPAT("??????? 00000 ????? 000 ????? 11000 11", beqz, B, s->dnpc = (src1 == 0) ? s->dnpc + imm : s->pc + 4;);
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B, s->dnpc = (src1 != src2) ? s->dnpc + imm : s->pc + 4;);
    INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B, s->dnpc = (src1 >= src2) ? s->dnpc + imm : s->pc + 4;);
