    ...
    if (!success) {
        do {
            if (!(
                ...
                assert_fail_msg();
                ((void)sizeof((1) ? 1 : 0), __extension__({ if (1) ; else __assert_fail (
       "1"
       , "src/monitor/sdb/sdb.c", 106, __extension__ __PRETTY_FUNCTION__); }));
            }
        } while (0);
    } else {
        printf("%d\n", res);
...
void test_expr() {
    ...
    while (
        ...
        ((void)sizeof((success) ? 1 : 0), __extension__({ if (
       success
       ) ; else __assert_fail (
       "success"
       , "src/monitor/sdb/sdb.c", 279, __extension__ __PRETTY_FUNCTION__); }));
        if (res != correct_res) {
            puts(e);
           ...
           "0"
           , "src/monitor/sdb/sdb.c", 283, __extension__ __PRETTY_FUNCTION__); }));
        }
    }
    fclose(fp);
    if (e) free(e);
    do {
        printf("\33[1;34m"
               ...
               "\33[0m"
               "\n",
               "src/monitor/sdb/sdb.c", 290, __func__);
        do {
            extern FILE *log_fp;
            extern _Bool
            log_enable();
