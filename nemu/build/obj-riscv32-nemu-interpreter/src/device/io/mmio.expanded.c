...
static IOMap *fetch_mmio_map(paddr_t addr) {
    ...
    return (mapid == -1 ? ((void *)0) : &maps[mapid]);
}
static void report_mmio_overlap(const char *name1, paddr_t l1, paddr_t r1, const char *name2, paddr_t l2, paddr_t r2) {
   ...
   "0"
   , "src/device/io/mmio.c", 30, __extension__ __PRETTY_FUNCTION__); }));
        }
    } while (0);
}
void add_mmio_map(const char *name, paddr_t addr, void *space, uint32_t len, io_callback_t callback) {
   ...
   "nr_map < NR_MAP"
   , "src/device/io/mmio.c", 37, __extension__ __PRETTY_FUNCTION__); }));
    paddr_t left = addr, right = addr + len - 1;
    if (in_pmem(left) || in_pmem(right)) {
        report_mmio_overlap(name, left, right, "pmem", ((paddr_t)0x80000000), ((paddr_t)0x80000000 + 0x8000000 - 1));
    }
    ...
    do {
        printf("\33[1;34m"
               ...
               "\33[0m"
               "\n",
               "src/device/io/mmio.c", 49, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high);
        do {
            extern FILE *log_fp;
            extern _Bool
            log_enable();
