    ...
    switch (len) {
        ...
        default:
            ((void)sizeof((0) ? 1 : 0), __extension__({ if (
                ...
                "0"
                , "/home/fisher/pa/nemu/include/memory/host.h", 29, __extension__ __PRETTY_FUNCTION__); }));
    }
}
static inline void host_write(void *addr, int len, word_t data) {
    switch (len) {
        ...
        default:
           ...
           "0"
           , "/home/fisher/pa/nemu/include/memory/host.h", 44, __extension__ __PRETTY_FUNCTION__); }))
...
uint8_t *new_space(int size) {
    ...
    ((void)sizeof((p_space - io_space < (32 * 1024 * 1024)) ? 1 : 0), __extension__({ if (
   p_space - io_space < (32 * 1024 * 1024)
   ) ; else __assert_fail (
    ...
    if (map ==
        ...
        do {
            if (!(map !=
                ...
                assert_fail_msg();
                ((void)sizeof((map != ((void *)0)) ? 1 : 0), __extension__({ if (
       map != 
       ...
       , "src/device/io/map.c", 37, __extension__ __PRETTY_FUNCTION__); }));
        ...
        do {
            if (!(addr <= map->high && addr >= map->low)) {
                ...
                extern void assert_fail_msg();
                assert_fail_msg();
                ((void)sizeof((addr <= map->high && addr >= map->low) ? 1 : 0), __extension__({ if (
...
static void invoke_callback(io_callback_t c, paddr_t offset, int len, _Bool is_write) {
    if (c !=
        ((void *)0)) {
...
void init_map() {
    io_space = malloc((32 * 1024 * 1024));

    ((void)sizeof((io_space) ? 1 : 0), __extension__({ if (
   io_space
   ) ; else __assert_fail (
   "io_space"
...
word_t map_read(paddr_t addr, int len, IOMap *map) {
    ...
    ((void)sizeof((len >= 1 && len <= 8) ? 1 : 0), __extension__({ if (
   len >= 1 && len <= 8
   ) ; else __assert_fail (
   "len >= 1 && len <= 8"
   , "src/device/io/map.c", 56, __extension__ __PRETTY_FUNCTION__); }));
...
void map_write(paddr_t addr, int len, word_t data, IOMap *map) {
   ...
   "len >= 1 && len <= 8"
   , "src/device/io/map.c", 65, __extension__ __PRETTY_FUNCTION__); }));
    check_bound(map, addr);
    paddr_t offset = addr - map->low;
